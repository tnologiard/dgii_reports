            ...
            callback: (r) => {
                ...
                if (docs_to_cancel.length > 0) {
                    ...
                    const dialog = new frappe.ui.Dialog({
                        ...
                        primary_action: (values) => {
                            // Proceder con la cancelación de los documentos
                            dgii_reports.bulk_operations.proceed_with_cancellation.call(this, docnames, action, values.tipo_de_anulacion, docs_to_cancel, done);
                            dialog.hide();
                        }
                    });
...
dgii_reports.bulk_operations.update_custom_codigo_de_anulacion = function(docs_to_update, tipo_de_anulacion) {
    return new Promise((resolve, reject) => {
        frappe.call({
            method: "frappe.client.get",
            args: {
            ...
            callback: (r) => {
                const codigo = r.message.codigo;
                const update_promises = docs_to_update.map(doc => {
                    return frappe.db.set_value(this.doctype, doc.name, 'custom_codigo_de_anulacion', codigo);
                });
...
dgii_reports.bulk_operations.proceed_with_cancellation = function(docnames, action, tipo_de_anulacion, docs_to_cancel, done) {
    ...
    return frappe
        ...
        .then((failed_docnames) => {
            ...
            // Manejar documentos exitosos
            if (successful_docnames.length) {
                // Actualizar el campo custom_codigo_de_anulacion para los documentos exitosamente cancelados
                dgii_reports.bulk_operations.update_custom_codigo_de_anulacion.call(this, successful_docnames.map(name => ({ name })), tipo_de_anulacion)
                    .then(() => {
                        frappe.utils.play_sound(action);
                        if (done) done.call(this);  // Asegurarse de que `this` se refiere al contexto correcto
                    })
                    .catch((error) => {
                        ...
                        frappe.msgprint({
                            title: __('Error'),
                            indicator: 'red',
                            message: __("Error updating custom_codigo_de_anulacion: {0}", [error.message])
                        });
                    });
            }
        })
